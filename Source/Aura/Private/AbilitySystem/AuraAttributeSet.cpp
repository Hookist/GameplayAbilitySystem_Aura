...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	{
		const float localIncomeDamage = GetIncomingDamage();
		SetIncomingDamage(0);
		if (localIncomeDamage > 0.f)
		{
			const float newHealth = GetHealth() - localIncomeDamage;
			...
			ShowFloatingText(props, localIncomeDamage, bBlock, bCriticalHit);
		}
	}
	...
	if (Data.EvaluatedData.Attribute == GetIncomingXPAttribute())
	{
		...
		{
			const int32 currentLevel = ICombatInterface::Execute_GetCreatureLevel(props.SourceCharacter);
			...
			if (numLevelUps > 0)
			{
				...
				SetMana(GetMaxMana());
				// GetAttributePointsReward and
				// GetSpellPointsReward()
				// AddToPlayerLevel()
				// AddToAttributePointsAndSpellPoints()
...
void UAuraAttributeSet::OnRep_HealthRegenetration(const FGameplayAttributeData& OldHealthRegeneration) const
{
